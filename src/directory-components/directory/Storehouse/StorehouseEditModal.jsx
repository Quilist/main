...
export default function StorehouseEditModal({ open, setOpenEditModal, id }) {
  ...
  React.useEffect(() => {
    api.getStorehouse(id).then(data => {
  ...
  const handleSave = () => {
    ...
    api.editStorehouse(id, body).then(data => {
  ...
  const handleDelete = () => {
    api.removeStorehouse(id).then(data => {
      if (data.status === "error") return alert(data.message)
      handleCloseModal();
    })
    ...
    <div>
      ...
      >
        <Box sx={style} className={styles.childModal}>
          ...
          <div className={styles.modal_title}>Редактирование склада</div>
          <TextField
