...
export default function ExpenditureEditModal({ open, setOpenEditModal, id }) {
  ...
  React.useEffect(() => {
    api.find(id, 'expenditure').then(data => {
  ...
  const handleSave = () => {
    ...
    api.edit(id, body, 'expenditure').then(data => {
  ...
  const handleDelete = () => {
    api.remove(id, 'expenditure').then(data => {
      if (data.status === "error") return alert(data.message)
      handleCloseModal();
    })
