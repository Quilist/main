...
function WriteOff() {
  ...
  React.useEffect(() => {
    api.auxiliary('productWriteOff').then(data => {
      if (data.status === "error") alert(data.message)
      else setAuxiliaryList(data.message)
    })

    if(id) {
      api.find(id, 'productWriteOff').then(data => {
  ...
  const handleAdd = () =>  {
    ...
    if(!id) {
      api.add(data, 'productWriteOff').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
    } else {
      api.edit(id, data, 'productWriteOff').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
  ...
  const handleRemove = () => {
    setTimeout(() => {
      api.remove(id, 'productWriteOff').then(data => {
