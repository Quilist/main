...
function Import() {
  ...
  React.useEffect(() => {
    api.auxiliary('productImport').then(data => {
      if (data.status === "error") alert(data.message)
      else setAuxiliaryList(data.message)
    })

    if(id) {
      api.find(id, 'productImport').then(data => {
  ...
  const handleAdd = () =>  {
    ...
    if(!id) {
      api.add(data, 'productImport').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
    } else {
      api.edit(id, data, 'productImport').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
  ...
  const handleRemove = () => {
    setTimeout(() => {
      api.remove(id, 'productImport').then(data => {
