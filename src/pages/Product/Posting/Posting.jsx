...
function Posting() {
  ...
  React.useEffect(() => {
    api.auxiliary('productPosting').then(data => {
      if (data.status === "error") alert(data.message)
      else setAuxiliaryList(data.message)
    })

    if(id) {
      api.find(id, 'productPosting').then(data => {
  ...
  const handleAdd = () =>  {
    ...
    if(!id) {
      api.add(data, 'productPosting').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
    } else {
      api.edit(id, data, 'productPosting').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
  ...
  const handleRemove = () => {
    setTimeout(() => {
      api.remove(id, 'productPosting').then(data => {
