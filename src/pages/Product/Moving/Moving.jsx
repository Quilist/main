...
function Moving() {
  ...
  React.useEffect(() => {
    api.auxiliary('productMoving').then(data => {
      if (data.status === "error") alert(data.message)
      else setAuxiliaryList(data.message)
    })

    if(id) {
      api.find(id, 'productMoving').then(data => {
  ...
  const handleAdd = () =>  {
    ...
    if(!id) {
      api.add(data, 'productMoving').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
    } else {
      api.edit(id, data, 'productMoving').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
  ...
  const handleRemove = () => {
    setTimeout(() => {
      api.remove(id, 'productMoving').then(data => {
