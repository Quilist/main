...
function ManufactureHandle() {
  ...
  React.useEffect(() => {
    api.auxiliary('manufacture').then(data => {
      if (data.status === "error") alert(data.message)
      else setAuxiliaryList(data.message)
    })

    if(id) {
      api.find(id, 'manufacture').then(data => {
  ...
  const handleAdd = () =>  {
    ...
    if(!id) {
      api.add(data, 'manufacture').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
    } else {
      api.edit(id, data, 'manufacture').then(data => {
        if (data.status === "error") return console.log(data.message)
        setIsSuccess(null)
        setIsRedirect(true)
      })
  ...
  const handleRemove = () => {
    setTimeout(() => {
      api.remove(id, 'manufacture').then(data => {
...
export default ManufactureHandle;
